...
namespace FKala.Core.DataLayer.Cache
{
    public class Cache_5Minutely : Cache_Base, ICache
    {
        ...
        public Window Window { get; } = Window.Aligned_5Minutes;
        ...
        {
            var baseQuery = new Op_Load("", "fullRes", measurement, start, end, CacheResolutionPredefined.NoCache);
            baseQuery.DontInvalidateCache_ForUseWhileCacheRebuild = true;
            KalaResult aggResult = KalaQuery
               .Start()
               .Add(baseQuery)
               .Add(new Op_Aggregate("", "5minutely", "fullRes", Window, aggrFunc, false, false))
               .Add(new Op_Publish("", new List<string>() { "5minutely" }, PublishMode.MultipleResultsets))
               .Execute(DataLayer);
        ...
        public override DateTime ShouldUpdateFromWhere(int cacheYear, DataPoint? newestInCache, DataPoint? newestInRaw)
        {
            ...
            if (newestInCache.StartTime.Add(new TimeSpan(0, 10, 0)) > newestInRaw.StartTime)
            {
                return DateTime.MaxValue;
            }

            // then refresh the last 15 (3 windows) existent minutes in the cache
            DateTime calculated = newestInCache.StartTime.Subtract(new TimeSpan(0, 15, 0));
